...
loading_val_stress = np.array([0.1, 137.8e+6, 206.7e+6, 275.6e+6, 344.5e+6, 413.4e+6, 438.9e+6, 454.8e+6, 482.3e+6, 551.2e+6, 620.1e+6, 689.1e+6, 758.0e+6, 808.3e+6])
...
for i in range(len(pts_list)):
    ...
    if worldcomm.size == 1:
        #########################################################################################################
        #for single run we only need mesh and do not care about the loading
        #########################################################################################################
        # loading_val = [0.1]
...
# print(np.array(mesh_size_CG1[0].vector().get_local())*1000)
...
for load_cycle in range(len(loading_val)):
    ...
    f_l = as_vector([Constant(-loading_val[load_cycle]/SR), Constant(0.), Constant(0.)])
    f_r = as_vector([Constant(loading_val[load_cycle]/SR), Constant(0.), Constant(0.)])
    ...
    for i in range(len(splines)):
        ...
        if i == 0 or i == 2:
            source_terms[i] += distr_loading(problem, f_l, left_bdry, i)
        else:
            source_terms[i] += distr_loading(problem, f_r, right_bdry, i)
